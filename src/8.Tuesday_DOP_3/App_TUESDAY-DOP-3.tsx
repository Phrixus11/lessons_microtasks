...
function App_TUESDAY_DOP_3() {
    // ТРИ задания от простого к сложному:
    // 1. Не работает кнопка удаления маршрута (routes) в App ничего править не нужно -ok
    // 2. Не работает ЧЕКБОКС -ошибки даже в APP! Вместо того чтобы передавать значение,в функции перещелкивается противоположное! -ok
    // 3. Обновление МАРШРУТА И ДАТЫ научились работать без функции в App, это нормально? Но в App ничего править не нужно
    // 3. Вначале почини ДАТУ, а потом убедись, что и ОБНОВЛЕНИЕ МАРШРУТА "починилось" каким-то волшебным образом, но так ли это?
    ...
    const toggleFTIsBooked = (flightTableID: string, routeID: string, newStatusIsBooked: boolean) => {
        setFlightTables(flightTables.map(ft =>
            ft.flightTableID === flightTableID ? {
                    ...ft,
                    routes: ft.routes.map(route =>
                        route.id === routeID ? {...route, isBooked: newStatusIsBooked} : route
                    )
                }
                : ft
        ));
...
export default App_TUESDAY_DOP_3;